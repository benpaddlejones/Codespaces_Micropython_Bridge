...
| F10  | ❌ Reassessed (by design) | After studying the Arduino `micropython.js` reference, this matches gold-standard practice: Arduino also uses `.toString()` for the text terminal and byte-encoding only for file transfers. Our file push/pull is already base64-encoded (binary-safe, see F17a), so the terminal `.toString()` path is correct as-is. No change made — a full binary terminal refactor is unjustified regression risk on unvalidated hardware. |
| F12  | ✅ Implemented            | Subsumed by F17: each chunked upload now waits for the device's `\x04>` frame acknowledgement (`sendRawCommandUntilMarker`) before sending the next chunk, giving synchronous request/response backpressure.                                                                                                                                                                                                                     |
| F13  | ✅ Implemented            | Device command titles reworded with "(in Browser)" (runFile/uploadFile/uploadProject/listFiles/openREPL/softReset/hardReset/stopCode) to signal the browser hand-off. Confirmed in `src/commands/index.ts` that each only calls `server.openInBrowser()`.                                                                                                                                                                        |
| F14  | ✅ Partially / ❌ rest    | Device-side rename + implicit mkdir added to the Device Files panel (`syncStatus.js` → `os.rename`/`ensureDirectory`). `mip`/package install deferred by design — the Arduino lab offloads it to a separate app; a half-baked impl would over-engineer. All other "gaps" already exist in the browser UI.                                                                                                                        |
| F16  | ❌ Invalid (by design)    | Behaviour kept by design; logging is already bounded via `/api/resilience`.                                                                                                                                                                                                                                                                                                                                                      |
| F17  | ✅ Implemented            | Raw-REPL now uses the Arduino-style read-until-marker handshake: double Ctrl-C interrupt, wait for `raw REPL; CTRL-B to exit` banner, complete on the universal `\x04>` frame terminator (or injected marker) instead of blind `sleep()`, with a sleep fallback so timing never regresses. `parseRawResult` surfaces device tracebacks.                                                                                          |
| F17a | ✅ Implemented            | Single-file + batch uploads now open `'wb'` and feed `a2b_base64(...)` bytes directly (was `'w'` + `.decode()`), matching the already-binary large-file path so non-UTF-8 content round-trips byte-for-byte.                                                                                                                                                                                                                     |

---

## Outstanding tasks

> **All previously-outstanding items are now addressed** (see the Implementation status table above), informed by a comparison against the Arduino `micropython.js` / lab-micropython-editor reference. **Caveat:** every change below touches the live serial/REPL/upload data path and still needs **physical device hardware to validate** before merging — none could be exercised against real hardware in this environment.

| #   | ID   | Status          | Outcome                                                                                                                                                                                                                                                                                                |
| --- | ---- | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | F17  | ✅ Implemented  | Adopted the Arduino-style **plain raw REPL read-until-marker** handshake (not raw-paste — lower risk, same flow-control benefit). Double Ctrl-C interrupt → wait for `raw REPL; CTRL-B to exit` banner → complete on the universal `\x04>` frame terminator or injected marker, with a sleep fallback. |
| 2   | F10  | ❌ Reassessed   | Left unchanged by design. Arduino itself uses `.toString()` for the text terminal and byte-encoding only for file transfers — exactly our split (push/pull are base64). A full binary terminal refactor is unjustified regression risk on unvalidated hardware.                                        |
| 3   | F12  | ✅ Implemented  | Subsumed by F17: each chunk waits for the device's `\x04>` ack before the next is sent (synchronous request/response backpressure).                                                                                                                                                                    |
| 4   | F17a | ✅ Implemented  | Single-file + batch uploads now use `'wb'` + raw `a2b_base64(...)` bytes, matching the already-binary large-file path.                                                                                                                                                                                 |
| 5   | F14  | ✅ / ⏳ partial | Device-side rename + implicit mkdir added to the Device Files panel. `mip`/package install deferred by design (the Arduino lab offloads it to a separate app).                                                                                                                                         |
| 6   | F13  | ✅ Implemented  | Device command titles reworded with "(in Browser)" to signal the browser hand-off.                                                                                                                                                                                                                     |
